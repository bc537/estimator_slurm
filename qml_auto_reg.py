...
newpath = f'regression_models/{initial_layer}_{entangling_layer}/{two_local_initial_layer}_{two_local_entangling_layer}_{ansatz_entanglement}/split{split}/a{ansatz_layers}_m{int(maxiter/1000)}k'
...
for i in range(num_points):
    ...
    estimator_classifier = NeuralNetworkRegressor(estimator_qnn, optimizer=COBYLA(maxiter=maxiter), callback=callback, initial_point=initial_point)
    ...
    training_scores.append(estimator_classifier.score(train_circuits, df_train_property))
    test_scores.append(estimator_classifier.score(test_circuits, df_test_property))
